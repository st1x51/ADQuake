 ...
 *
 *
 *  $Id: pr2_vm.c,v 1.19 2006/03/22 15:28:37 angel Exp $
 */
/*
...
#include "quakedef.h"
#include <pspkernel.h>
#include "psp/m33libs/kubridge.h"
#include "g_public.h"
#define DLEXT	"prx"
#ifdef USE_PR2
...
void VM_Unload( vm_t * vm )
{
	...
	Q_snprintfz( name, sizeof( name ), "%s." DLEXT, vm->name );
	if ( !vm )
		return;
	Con_Printf( "VM_Unload \"%s\"\n", vm->name );
	...
	case VM_NATIVE:
		...
		{
			int status;
			int ret = sceKernelStopModule(libHandle, 0, 0, &status, NULL);	
			if(ret < 0)
				Sys_Error("Sys_UnloadDll: Error stop module (%08x)", ret);
			ret = sceKernelUnloadModule(libHandle);
			if(ret < 0)
...
SceUID Sys_DLOpen (const char *prxname)
{
	//SceUID libHandle;
	char    name[MAX_OSPATH];
	Q_snprintfz( name, sizeof( name ), "%s", prxname );
	libHandle = pspSdkLoadStartModule(name, PSP_MEMORY_PARTITION_USER);
	if(libHandle < 0)
	{
		Con_Printf("Sys_LoadPRX: pspSdkLoadStartModule failed (%08x), trying kuKernelLoadModule\n", libHandle);
		libHandle = kuKernelLoadModule(name, 0, NULL);
		if(libHandle < 0)
		{
			Con_Printf( "Sys_LoadPRX(%s) failed: %08x\n", name, libHandle);
	        return 0;
		}
		else
		{
			...
			if (ret < 0)
			{
			    Con_Printf("Sys_GetGameAPI: Error start module (%08x)", ret);
			    return 0;
			}
		}
	}
...
qboolean VM_LoadNative( vm_t * vm )
{
	...
	{
		VM_Unload( vm );
		Sys_Error( "VM_LoadNative: couldn't initialize module %s", name );
	}
...
void LoadMapFile( qvm_t*qvm, char* fname )
{
	...
	Q_snprintfz( name, sizeof( name ), "%s.map", fname );
...
qboolean VM_LoadBytecode( vm_t * vm, sys_callex_t syscall )
{
	...
	Q_snprintfz( name, sizeof( name ), "%s.qvm", vm->name );
...
vm_t   *VM_Load( vm_t * vm, vm_type_t type, const char *name, sys_call_t syscall, sys_callex_t syscallex )
{
	...
	do
	{
		...
		case OP_ENTER:
			qvm->LP -= op.parm._int;
           		...
           		if ( qvm->LP < qvm->len_ds - qvm->len_ss )
           		{
           			QVM_RunError( qvm, "QVM Stack overflow on enter at %8x", qvm->PC );
           		}
#endif
